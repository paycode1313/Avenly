...
import { Map, Share2, User, Bell } from 'lucide-react';
...
export default function Layout() {
  ...
  const isNavigateRoute = location.pathname === '/navigate';

  return (
    <div className="flex flex-col h-screen bg-white text-zinc-900">
      {/* Top Banner — hidden on /navigate for full-screen map experience */}
      {!isNavigateRoute && (
        <motion.header
          initial={{ y: -100 }}
          animate={{ y: 0 }}
      ...
      {/* Bottom Navigation — hidden on /navigate for full-screen map experience */}
      {!isNavigateRoute && (
        <motion.nav
          initial={{ y: 100 }}
          animate={{ y: 0 }}
          transition={{ type: "spring", stiffness: 100, delay: 0.1 }}
          className="bg-white/95 backdrop-blur-2xl border-t border-zinc-200 pb-safe pt-2 px-6 flex justify-around items-center z-50 shadow-[0_-2px_20px_rgba(0,0,0,0.08)]"
        >
    ...
    >
      {({ isActive }) => (
        <>
          ...
          >
            {icon && (icon.props as React.Attributes & { className?: string }).className === undefined
                  ? React.cloneElement(icon, { className: "w-5 h-5" } as React.HTMLAttributes<Element> & React.ClassAttributes<Element>)
                  : icon}
            {isActive && (
              <motion.div
                layoutId="activeTab"
